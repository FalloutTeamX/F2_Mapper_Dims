...
void TfrmConfig::ShowDialog(void)
{
   ...
   this->chb->Checked = pUtil->StringConvert;
...
void __fastcall TfrmConfig::btnOKClick(TObject *Sender)
{
   ...
   pUtil->SetRegInfo("\\SOFTWARE\\Dims\\mapper", "GamePath", pUtil->GameDir);
   pUtil->DataDir = this->edDataDir->Text;
   pUtil->SetRegInfo("\\SOFTWARE\\Dims\\mapper", "DataPath", pUtil->DataDir);
