...
CUtilites::CUtilites(void)
{
   ...
   DataDir = "";
   GameDir = "";
   StringConvert = false; // by default we don't need convert strings
   char TempBuf[256];
...
bool CUtilites::GetRegInfoBool(String RegKey, String KeyName)
{
    ...
    try
    {
       if(MyRegistry->OpenKey(RegKey, false))
       {
          RetVal = MyRegistry->ReadBool(KeyName);
          MyRegistry->CloseKey();
       }
    }
    catch(ERegistryException &E)
...
//--------------------------------------------------------------------------
bool CUtilites::SetRegInfo(String RegKey, String KeyName, String Value)
{
    bool RetVal = false;
    TRegistry *MyRegistry = new TRegistry();
    MyRegistry->RootKey = HKEY_LOCAL_MACHINE;
...
bool CUtilites::SetRegInfo(String RegKey, String KeyName, bool Value)
{
    bool RetVal = false;
    TRegistry *MyRegistry = new TRegistry();
    MyRegistry->RootKey = HKEY_LOCAL_MACHINE;
    try
    {
       if(MyRegistry->OpenKey(RegKey, true))
       {
          MyRegistry->WriteBool(KeyName, Value);
          MyRegistry->CloseKey();
          RetVal = true;
       }
    }
                                                                    ...
                                                                    int *iLevel)
{
   ...
   *iLevel = Y / 100;
   Y = Y % 100;
   *WorldX = 4752 - (48 * X) + (Y << 5) - 320;
   *WorldY = (12 * X) + (24 * Y) - 240;
}
//------------------------------------------------------------------------------
...
//  *WorldY = ((12 * (X >> 1)) + (Y * 12)) + 11; // + 8
  *WorldX = 4816 - ((((X + 1) >> 1) << 5 )+ ((X >> 1) << 4) - (Y << 4)); //ver1
  *WorldY = ((12 * (X >> 1)) + (Y * 12)) + 11; // + 8
}
//------------------------------------------------------------------------------
